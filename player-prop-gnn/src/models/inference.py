...
class BayesianPredictor:
    ...
    def _load_model(self):
        ...
        logger.info(f"✓ Cached samples ready")
    ...
    def _cache_posterior_samples(self, idata: az.InferenceData):
        ...
        if self.n_samples > n_total:
            logger.warning(f"Requested {self.n_samples} samples but only {n_total} available. Using {n_total}.")
    ...
    def predict_player(
        ...
        Args:
            ...
            position: Player position (Forward/Midfielder/Defender)
            was_home: True if home game
            features: Dict of feature values
                Required keys: goals_rolling_5, shots_on_target_rolling_5,
        ...
        Returns:
            Dict with structure:
            {
                'goals': {
                    ...
                    'ci_low': 0,
                    'ci_high': 2,
                    'probability': {'0': 0.64, '1': 0.29, '2+': 0.07}
                },
                'shots': {...},
                'cards': {...},
                'metadata': {
        ...
        # Note: In production, should use training mean/std from metadata
        # For now, features are already standardized in database
        ...
        try:
            lambda_goals = self._predict_poisson(
                ...
                gamma=self.cached_samples['gamma_goals'][:, opp_idx] if opp_idx is not None else self.cached_samples['gamma_goals'].mean(axis=1),
                beta=self.cached_samples['beta_goals'],
                X=X,
                clip_min=-6,
                clip_max=1
            )
            goals_samples = np.random.poisson(lambda_goals)
        ...
        try:
            lambda_shots = self._predict_poisson(
                ...
                gamma=self.cached_samples['gamma_shots'][:, opp_idx] if opp_idx is not None else self.cached_samples['gamma_shots'].mean(axis=1),
                beta=self.cached_samples['beta_shots'],
                X=X,
                clip_min=-6,
                clip_max=2
            )
            shots_samples = np.random.poisson(lambda_shots)
        ...
        try:
            lambda_cards = self._predict_poisson(
                ...
                gamma=self.cached_samples['gamma_cards'][:, opp_idx] if opp_idx is not None else self.cached_samples['gamma_cards'].mean(axis=1),
                beta=self.cached_samples['beta_cards'],
                X=X,
                clip_min=-6,
                clip_max=0
            )
            cards_samples = np.random.poisson(lambda_cards)
